...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        Rx = myrobot.x - spath[index][0]    #estimate[0]
        Ry = myrobot.y - spath[index][1]    #estimate[1]
        dx = spath[index+1][0] - spath[index][0]
        dy = spath[index+1][1] - spath[index][1]
        u = (Rx * dx + Ry * dy) / sqrt(dx ** 2 + dy ** 2) 
        ### ENTER CODE HERE
        if (u>1 and index <= len(spath) -1):              #solution has index < len(spath)-1 
            index += 1
            print (index)
            Rx = myrobot.x - spath[index][0]                #estimate[0]
            Ry = myrobot.y - spath[index][1]               #IN answer estimate[1]
            dx = spath[index+1][0] - spath[index][0]
            dy = spath[index+1][1] - spath[index][1]
            u = (Rx * dx + Ry * dy) / sqrt(dx ** 2 + dy ** 2)   
